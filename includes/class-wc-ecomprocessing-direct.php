...
if ( ! class_exists( 'WC_Ecomprocessing_Method_Base' ) ) {
	require_once dirname( __DIR__ ) . '/classes/wc_ecomprocessing_method_base.php';
}
 ...
 */
class WC_Ecomprocessing_Direct extends WC_Ecomprocessing_Method_Base {
	...
	protected function get_module_title() {
		return static::get_translated_text( 'ecomprocessing Direct' );
	}
	...
	protected function get_checkout_transaction_id_meta_key() {
		return WC_Ecomprocessing_Order_Factory::is_hpos_enabled() ? self::META_HPOS_DIRECT_TRANSACTION_ID : self::META_TRANSACTION_ID;
	}
	...
	protected function is_applicable() {
		return parent::is_applicable() &&
			WC_Ecomprocessing_Helper::is_store_over_secured_connection();
	}
	...
	public function init_form_fields() {
		...
		$this->method_description =
			static::get_translated_text( 'ecomprocessing\'s Gateway offers a secure way to pay for your order, using Credit/Debit Card.' ) .
			'<br />' .
		...
		$this->form_fields += array(
			self::SETTING_KEY_TOKEN             => array(
				...
				'description' => static::get_translated_text(
					...
					'If you do not have a token, contact tech-support@e-comprocessing.com ' .
	...
	protected function process_order_payment( $order_id ) {
		...
		try {
			...
			if ( ! WC_ecomprocessing_Subscription_Helper::is_init_gateway_response_successful( $response ) ) {
			...
			wc_ecomprocessing_order_proxy()->set_order_meta_data( $order, $this->get_checkout_transaction_id_meta_key(), $response->unique_id );
			wc_ecomprocessing_order_proxy()->set_order_meta_data( $order, self::META_TRANSACTION_TYPE, $response->transaction_type );
			...
			switch ( true ) {
				case ( isset( $response->threeds_method_url ) ):
					...
					$response_array = $this->create_response(
						WC()->api_request_url( WC_Ecomprocessing_Threeds_Form_Helper::class ) . "?{$url_params}",
	...
	protected function prepare_initial_genesis_request( $data ) {
		$genesis = WC_ecomprocessing_Genesis_Helper::get_gateway_request_by_txn_type( $data['transaction_type'] );
		...
		$is_recurring = WC_ecomprocessing_Subscription_Helper::is_init_recurring(
	...
	protected function process_init_subscription_payment( $order_id ) {
		...
		try {
			...
			$response = $genesis->response()->getResponseObject();
			wc_ecomprocessing_order_proxy()->save_initial_trx_to_order( $order, $response, $data );
			...
			switch ( true ) {
				...
				default:
					...
					if ( ! $this->process_after_init_recurring_payment( $order, $response ) ) {
						throw new Exception( WC_Ecomprocessing_Genesis_Helper::fetch_gateway_response_message( $response ) );
					}
					$woocommerce->cart->empty_cart();
					$response_array = $this->create_response( $data['return_success_url'] );
			}
	...
	protected function set_terminal_token( $order ) {
		...
		$terminal_token = wc_ecomprocessing_order_proxy()->get_order_meta_data( $order, self::META_TRANSACTION_TERMINAL_TOKEN );
		...
		$recurring_token = wc_ecomprocessing_order_proxy()->get_order_meta_data( $order, WC_ecomprocessing_Subscription_Helper::META_RECURRING_TERMINAL_TOKEN );
	...
	protected function save_direct_trx_data_to_order( $response_obj, $order, $data ) {
		wc_ecomprocessing_order_proxy()->save_initial_trx_to_order( $order, $response_obj, $data );

		// Empty token will mean that Smart Routing is used
		wc_ecomprocessing_order_proxy()->set_order_meta_data( $order, self::META_TRANSACTION_TERMINAL_TOKEN, $this->get_method_setting( self::SETTING_KEY_TOKEN ) );
